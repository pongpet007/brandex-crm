            ...
            <tr class="{{ $quotation->is_active == 1 ? 'bg-success bg-opacity-10' : '' }}">
                ...
                <td class="">
                    ...
                    @if ($quotation->is_active == 1)
                        <button type="button" wire:confirm=' set to inactive ?' wire:click='setinactive({{ $quotation->quote_id }})'
                            class="btn btn-sm btn-success">set inactive</button>
                    @else
                        <button type="button" wire:confirm=' set to active ?' wire:click='setactive({{ $quotation->quote_id }})'
                            class="btn btn-sm btn-danger">set cctive</button>
                    @endif
